...
export function DocumentChecklistNew({ packageId, documents }: DocumentChecklistProps) {
  ...
  const deleteFileMutation = useMutation({
    mutationFn: async (documentId: number) => {
      return apiRequest("DELETE", `/api/documents/${documentId}/file`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/packages/${packageId}`] });
      queryClient.invalidateQueries({ queryKey: ['/api/packages'] });
  ...
  const DocumentRow = ({ doc }: { doc: PackageDocument }) => (
          ...
          {doc.fileName ? (
            <>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setViewingDocument({ filename: doc.fileName!, documentName: doc.documentName })}
                title="View file"
              >
